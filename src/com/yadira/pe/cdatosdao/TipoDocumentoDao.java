...
public class TipoDocumentoDao {
    ...
    public String agregarTipoDocumento(Connection conn, TipoDocumento tipoDocumento) {
        String sql = "INSERT INTO TIPO_DOCUMENTO (ID_TIPO_DOCUMENTO, nombre, siglas, nacionalidad, estado, fecha) VALUES (?, ?, ?, ?, ?, ?)";
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, tipoDocumento.getID_TIPO_DOCUMENTO());
            ps.setString(2, tipoDocumento.getNombre());
            ps.setString(3, tipoDocumento.getSiglas());
            ps.setString(4, tipoDocumento.getNacionalidad());
